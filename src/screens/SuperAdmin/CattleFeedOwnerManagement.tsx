...
import {
  ...
  approveCattleFeedOwner,
  addCattleFeedOwner,
  updateCattleFeedOwner,
  deleteCattleFeedOwner
} from '../../utils/storage';
...
const CattleFeedOwnerManagement: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      if (activeTab === 'active') {
        const data = await getCattleFeedOwners();
        setOwners(Array.isArray(data) ? data : []);
      } else {
        const data = await getPendingCattleFeedOwners();
        setPendingOwners(Array.isArray(data) ? data : []);
      }
    } catch (error: any) {
      console.error('Error loading cattle feed owners:', error);
  ...
  const handleSubmit = async () => {
    ...
    if (formData.username) ownerData.username = formData.username;

    try {
      if (editingOwner) {
        await updateCattleFeedOwner(editingOwner._id || editingOwner.id, ownerData);
        success('Owner updated successfully');
      } else {
        await addCattleFeedOwner(ownerData);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await approveCattleFeedOwner(owner._id || owner.id);
  ...
  const handleDelete = async (id: string) => {
    Alert.alert(
      'Delete Owner',
      'Are you sure you want to delete this cattle feed owner?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Delete',
          ...
          onPress: async () => {
            try {
              await deleteCattleFeedOwner(id);
  ...
  const resetForm = () => {
    setFormData({ name: '', username: '', password: '', phone: '', email: '', companyName: '' });
  ...
  const filteredOwners = Array.isArray(currentList) ? currentList.filter(owner => {
    ...
    return (
      ...
      (owner.username && owner.username.toLowerCase().includes(term)) ||
      (owner.phoneNumber && owner.phoneNumber.includes(searchTerm)) ||
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Cattle Feed Owners</Text>
        <Button
          variant="primary"
          onPress={() => {
            resetForm();
            ...
            {filteredOwners.map(owner => (
              <View key={owner._id || owner.id} style={styles.listItem}>
                <View style={styles.listItemContent}>
                  ...
                  <Text style={styles.phone}>{owner.phoneNumber || owner.phone}</Text>
...
export default CattleFeedOwnerManagement;
